...
original = table
original.dress = dress  ;   % read the dress iamge file into Matlab and assign it to the handle 'dress' 
...
combinedDressVector = [{blurredDress30LG} , {blurredDress25LG } , {blurredDress20LG} , {blurredDress15LG} , ...   ;
...
%%
combinedDressVector = [{blurredDress30} , {blurredDress25 } , {blurredDress20} , {blurredDress15} , ...   ;
...
str = {'Welcome Participant!',' ', ...
    ...
    ' ', 'Exit at anytime by pressing the ESC key.'} ;
...
for rr = 1:2
    ...
    responseMatrix(rr:rr+10, 5) = randomizedImageOrder' ;
    
  for ss = 1:length(combinedDressVector)                        % For a vector the length of 'sharpenFactorVec'
      presentationFigure = figure ;
      leftOrRight = randsample(leftRightRand,numOfSample)                         ;   % Randomly select a one or a two
      if leftOrRight == 1
          combinedImage = [dress combinedDressVector{ss}] ;
          responseMatrix(ss,4) = leftOrRight ;
          tic ;
          image(combinedImage)
          axis equal
          axis off
          ...
          responseMatrix(ss,4) = 1 ;
          combinedImage = [combinedDressVector{ss} dress] ;
          responseMatrix(ss,4) = leftOrRight ;
          tic ;
          image(combinedImage)
          axis equal
...
for i = 1:10
    for ii = randomizedImageOrder
        presentationFigure = figure ;
        image(combinedDressVector{ii})
        hold on
        axis equal
        axis off
        %pause
        simTool = randsample(leftRightRand,numOfSample) ;
        if simTool == 1
            responseMatrix(counter,2) = simTool ;
            responseMatrix(counter,1) = counter  ;
            counter = counter+1  ;
        elseif simTool == 2
            responseMatrix(counter,2) = simTool;
...
close all
% responseMatrix = responseMatrix(:,2) - 1
responseMatrix
...
str = {'Welcome Participant!',' ', ...
    'This experiment will present two images side by side that vary in degrees of blurriness.', ...
    'Please select which image you believe is *more blue* by using the keyboard.',...
    'Please do so by using the arrow keys; left arrow for the image on the left,', ...
    'and right arrow for the image on the right. You will be presented the image', ...
    'for 2 seconds only and can make a selection whenever you are ready.', ...
    ' ', ' ', 'Begin the experiment by pressing the spacebar.'}

textHandle = text(0.5,0.5,str)
axis off
textHandle.FontSize = 20; %bigger font
textHandle.HorizontalAlignment = 'center'; %Centered text
presentationFigure.Color = 'k' %[.9 .9 .9]; %Black background
textHandle.Color = 'g'; %Green text



for i = randomizedImageOrder
    subplot(3,4,i) 
    image(combinedDressVector{i})
    axis equal
...
blurFactor = 20; % Something reasonable
...
image(round(highFrequencyDress.*amplificationFactor)) %Bright, edge-enhanced, high spatial frequency version of the dress
axis equal
