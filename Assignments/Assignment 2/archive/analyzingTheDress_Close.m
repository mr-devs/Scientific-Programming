...
for rr = 1:repetitions                                                  % Iterate through a vector, 1 through 10 (these are the condition presentation repetitions)
    vectorPopulation = linspace(1,11,11)                            ;   % Create an 11 element vector from 1 to 11 that counts by one
    randomizedImageOrder = randsample(vectorPopulation, 11)         ;   % Create another vector that is a random sample of the above vector (the order 1 through 11 is now scrambled)
    ...
    for ai = 1:length(dressVector)                                      % For a vector the length of 'dressVector' (1 through 11) where ai will serve as the indexing value to draw from the randomizedImageOrder vector
        ...
        if leftOrRight == 1                                                             % If a 1 is selected
            ...
            presentationFigure = figure                                             ;   % Open a figure
...
% Initilaze the below variables for the loop below based on the different
% condition values.
...
for conditions = 1:length(numConditions)                                                            % For 11 conditions
    for rows = 1:length(numTrials)                                                                  % For 110 rows 
...
legend( 'Mean Proportion Chosen as More Blue', 'Location', 'EastOutside')            ;   % Legend description and location
